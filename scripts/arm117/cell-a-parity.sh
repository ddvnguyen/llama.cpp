...
greedy() { # $1 tag, $2 req file
  ...
  python3 -c 'import json,sys; d=json.load(open(sys.argv[1])); c=d["choices"][0]["message"]; print((c.get("reasoning_content") or "")+(c.get("content") or ""))' "$OUT/resp-$1.json" > "$OUT/text-$1.txt" 2>/dev/null || echo RESPONSE_PARSE_FAIL > "$OUT/text-$1.txt"
}
...
print(json.dumps({"messages":[{"role":"user","content":body}],"max_tokens":96,"temperature":0,"cache_prompt":True}))
PY
