...
// the whole-graph CUDA run takes the fused path (snapshot writes go straight to the cache and the
// trailing cpy is elided), so comparing the cpy node against the reference validates the fusion.
...
// Conv_2D test cases
 ...
 // A deep layer of a ConvNet, several images in the batch
    ...
    // gdn + cache cpy fusion (K > 1): whole-graph CUDA run must match the reference with the
    // trailing cpy elided (PR #112 / upstream d011a214b test, CUDA port of #23940 already in tree)
