...
ggml_status llama_context::graph_compute(
    ...
    if (status != GGML_STATUS_SUCCESS) {
        ...
        // #470 Option B: graph_compute failed — the most likely cause in
        // COMBINED mode is stale RPC buffers after a peer restart.  Set
        // peer_reconnection_pending unconditionally so the next update_slots
        // or PREFILL handler forces a T3 rebuild.
        LLAMA_LOG_WARN("%s: graph_compute failed (status=%d) — setting peer_reconnection_pending for T3 rebuild\n",
                            __func__, (int)status);
        peer_reconnection_pending = true;
    } else {
