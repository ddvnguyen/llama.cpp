...
void llama_model_qwen4exp::load_arch_tensors(llama_model_loader & ml) {
    ...
    // trunk_flags kept: a borrowing draft tolerates missing tensors
    ...
    for (int il = 0; il < (int) hparams.n_layer_all; ++il) {
        ...
        // grouped-norm gamma: new files store [n_embd, hc], old shared drafts store flat hc_dim;
        // ALLOW_RESHAPE accepts both (same element count), flags kept for borrowing drafts
